...
export const AiConciergeModal: React.FC = () => {
  ...
  // Sync watch history count and customize welcome message when user changes
  useEffect(() => {
    if (user?.id) {
      try {
        const pId = getActiveProfileId(user.id);
        const items = getProfileHistory(user.id, pId);
      ...
      setMessages([
        {
          id: "welcome-user",
          role: "assistant",
          content: `Hey ${user.username || "there"}! 🍿 I'm your **Be Chill AI Concierge**. I have access to your personal watch history and preferences. Tell me what mood, vibe, or genre you're feeling!`,
        },
      ]);
    } else {
      setHistoryCount(0);
      setMessages([
        {
          ...
          role: "assistant",
          content:
            "Welcome to **Be Chill AI Concierge**! 🍿\n\nPlease **sign in** to start chatting. Once logged in, I will use your personal watch history and taste to curate the best movies and TV series for you.",
        },
      ]);
    }
  ...
  const handleSend = async (textToSend?: string) => {
    ...
    if (!user) {
      setMessages((prev) => [
        ...
        },
        {
          ...
          role: "assistant",
          content:
            "🔒 **Sign In Required**\n\nPlease sign in to your Bu-Chill account to chat with the AI concierge and get personalized recommendations based on your watch history.",
        },
      ]);
      setInput("");
      return;
    ...
    try {
      ...
      try {
        const pId = getActiveProfileId(user.id);
        localHistory = getProfileHistory(user.id, pId);
      } catch {
        localHistory = [];
      }
      ...
      const response = await fetch("/api/ai/chat", {
        ...
        body: JSON.stringify({
          ...
          localHistory: localHistory.slice(0, 30).map((h) => ({
            title: h.title,
            type: h.type,
            progress: h.progress,
      ...
      setMessages((prev) => [
        ...prev,
        {
          ...
          role: "assistant",
          content:
            "Failed to connect to Be Chill AI. Please check your network connection.",
        },
      ]);
    } finally {
      setIsLoading(false);
        ...
        >
          ...
          >
            ...
            <div className="px-4 py-1.5 bg-black/30 border-b border-white/5 text-[11px] text-white/60 flex items-center justify-between">
              {user ? (
                <div className="flex items-center gap-1.5 text-amber-300/90 font-medium">
                  ...
                  <span>Personalized with {historyCount > 0 ? `${historyCount} watched titles` : "your watch history"}</span>
                </div>
              ) : (
                <div className="flex items-center justify-between w-full">
                  <span className="text-white/60 truncate flex items-center gap-1.5 font-medium">
                    <IoLockClosed className="w-3 h-3 text-amber-400" />
            ...
            <div className="flex-1 overflow-y-auto p-3.5 space-y-3 scrollbar-thin scrollbar-thumb-white/10">
              ...
              {/* If not logged in, render an interactive Sign In banner inside the chat */}
              {!user && (
                <div className="p-4 rounded-2xl bg-gradient-to-b from-amber-500/15 via-amber-500/10 to-transparent border border-amber-500/30 flex flex-col items-center text-center gap-3 my-2 shadow-xl backdrop-blur-sm">
                  <div className="w-12 h-12 rounded-full bg-amber-500/20 border border-amber-500/40 flex items-center justify-center text-amber-400 shadow-inner">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-white/70 text-xs max-w-[300px] leading-relaxed">
                      Only logged-in members can chat with Be Chill AI. Sign in to unlock instant AI recommendations powered by your real watch history and personal taste!
                    </p>
                  </div>
                  <Link
                    href="/auth"
              ...
              {QUICK_PROMPTS.map((prompt, i) => (
                <button
                  ...
                  className={cn(
                    ...
                    user
                      ? "bg-white/5 hover:bg-white/15 text-white/75 hover:text-white border-white/10 cursor-pointer"
                      : "bg-white/5 text-white/40 border-white/5 cursor-not-allowed opacity-60"
                  )}
                >
                  {prompt}
                </button>
            ...
            {/* Chat Input Capsule / Login Prompt */}
            <div className="p-3 border-t border-white/10 bg-[#0e0f17] shrink-0">
              {!user ? (
                ...
                >
                  <IoPersonCircleOutline className="w-4 h-4" />
                  <span>Sign in to chat with AI Concierge</span>
                </Link>
              ) : (
